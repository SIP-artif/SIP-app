...
class _LoginPageState extends State<LoginPageGuardian> {
  ...
  try {
    // Pass email and password as named parameters to loginGuardian
    final guardianId = await userService.loginGuardian(
      email: _emailController.text.trim(),  // Pass email
      password: _passwordController.text.trim(),  // Pass password
    );

    Navigator.pushReplacement(
      context,
      MaterialPageRoute(
        builder: (context) => GuardianHomePage(),  // Pass the guardian ID to the home page
      ),
    );
  } catch (error) {
  ...
  void _showErrorDialog(String message) {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        title: Text('Login Failed', style: GoogleFonts.lato(),),

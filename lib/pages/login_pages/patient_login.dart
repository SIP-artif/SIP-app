...
import 'package:myapp/services/user_service.dart'; // Import the user_service
...
class _LoginPageState extends State<LoginPage> {
  ...
  bool _isLoading = false; // To show loading spinner during authentication
...
Future<void> _handleLogin() async {
  ...
  try {
    final userId = await userService.loginUser(
      email: _emailController.text.trim(),
      password: _passwordController.text.trim(),
    ); // Await the result of the loginUser method

    // Proceed to home page if login was successful
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(
        builder: (context) => UserHomePage(userId: userId), // Pass the userId
      ),
    );
  } catch (error) {
    _showErrorDialog(error.toString()); // Show the error message
  }
